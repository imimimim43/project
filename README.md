...
- 위 사진 속 그래프는 서로 같은 그래프이지만 왼쪽 경로는 최대 유량이 2, 오른쪽 경로는 최대 유량이 1이다. 왼쪽 경로는 오른쪽 경로에서 유량이 1 늘었는데, 유량을 증강시키는 것을 **증강 경로(augmenting path)** 라고 한다. 포드-풀커슨 알고리즘의 목표는 왼쪽 경로와 같이 최대 유량을 찾는 것, 즉 증강 경로를 찾는 것이다.

> 증강 경로로 보낼 수 있는 최대 유량 = 포함된 간선의 잔여 용량 중에서 가장 작은 값.
